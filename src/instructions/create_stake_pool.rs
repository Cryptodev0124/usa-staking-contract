...
use crate::state::*;
use crate::state::{REWARD_ESCROW_A_PREFIX, REWARD_ESCROW_B_PREFIX, STAKE_POOL_PREFIX};
...
pub struct CreateStakePool<'info> {
    ...
    #[account(
        ...
        token::mint = mint_a,
        token::authority = stake_pool,
        seeds = [REWARD_ESCROW_B_PREFIX.as_bytes(), &mint_a.key().to_bytes(), &stake_pool.key().to_bytes()],
        bump
    )]
