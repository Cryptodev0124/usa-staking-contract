...
use crate::state::USA_DECIMALS;

pub fn calculate_rewards(
    rewards: u128,
    stake_amount: u128,
    current_rewards_per_token: u128,
    prev_rewards_per_token: u128,
) -> Result<u128, ProgramError> {
    return Ok(rewards
        + (current_rewards_per_token - prev_rewards_per_token) * stake_amount
            / (10_u128.pow(USA_DECIMALS)));
}

pub fn calculate_rewards_per_token(
    prev_rewards_per_token: u128,
    rewards_rate: u128,
    user_staked: u128,
    now_timestamp: u128,
    ...
    let interval = now_timestamp - prev_timestamp;
    return Ok(prev_rewards_per_token
        + interval * user_staked * (rewards_rate) * (10_u128.pow(USA_DECIMALS))
            / (total_staked)
            / 3110400000); //60 * 60 * 24 * 30 * 12 * 100
}
...
mod tests {
    ...
    fn stake(pool: &mut PoolEntry, rewards_entry: &mut RewardsEntry, amount: u128, timestamp: i64) {
        ...
        rewards_entry.rewards = calculate_rewards(
            ...
            pool.rewards_per_token_stored,
            rewards_entry.rewards_per_token_paid,
        ).unwrap();
    ...
    fn unstake(
        pool: &mut PoolEntry,
        rewards_entry: &mut RewardsEntry,
        amount: u128,
        timestamp: i64,
    ) {
        pool.rewards_per_token_stored = calculate_rewards_per_token(
    ...
    fn test_1() {
        ...
        let mut result = calculate_rewards_per_token(
            rewards_per_token,
            rewards_per_sec,
            now_timestamp as u128,
            prev_timestamp as u128,
            total_staked,
        ).unwrap();
